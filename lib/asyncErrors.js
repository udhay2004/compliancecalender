...
function errorHandler(err, req, res, next) { // eslint-disable-line no-unused-vars
  ...
  if (status >= 500) {
    ...
    require("./monitoring").captureError(err, { route: `${req.method} ${req.route ? req.baseUrl + req.route.path : req.path}` });
  } else {
    console.warn(`[warn] ${req.method} ${req.originalUrl}: ${status} ${err && err.message}`);
  }
