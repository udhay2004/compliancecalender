...
/**
 * Same file as dumpDatabase, but written bit by bit to a temporary file on
 * disk instead of being built in memory, and compressed without blocking
 * the server. Used for the real backups, so a large database can't freeze
 * or crash the site during the nightly backup.
 *
 * The manifest (document counts) is only known at the end, but must be the
 * first line. A gzip file may contain several compressed parts one after
 * another, which unzip as one text — so the documents are compressed into
 * one temp file, and the final file is [compressed manifest][compressed
 * documents].
 *
 * Returns { filePath, manifest, bytes, rawBytes, cleanup() }.
 */
async function dumpDatabaseToFile(db, { now = new Date() } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cc-backup-"));
  const docsFile = path.join(dir, "docs.gz");
  const finalFile = path.join(dir, "backup.ndjson.gz");
  ...
  try {
    ...
    async function* lines() {
      for (const name of names) {
        let n = 0;
        const found = db.collection(name).find({});
        const cursor = typeof found.batchSize === "function" ? found.batchSize(500) : found;
        for await (const doc of cursor) {
          const line = JSON.stringify({ c: name, d: JSON.parse(EJSON.stringify(doc, { relaxed: false })) }) + "\n";
    ...
    await pipeline(Readable.from(lines()), zlib.createGzip({ level: 6 }), fs.createWriteStream(docsFile));
    const manifest = { format: FORMAT, createdAt: now.toISOString(), database: db.databaseName || "", collections: counts };
    const head = JSON.stringify({ manifest }) + "\n";
    rawBytes += Buffer.byteLength(head);
    fs.writeFileSync(finalFile, zlib.gzipSync(Buffer.from(head, "utf8")));
    await pipeline(fs.createReadStream(docsFile), fs.createWriteStream(finalFile, { flags: "a" }));
    fs.rmSync(docsFile, { force: true });
    return { filePath: finalFile, manifest, bytes: fs.statSync(finalFile).size, rawBytes, cleanup };
  } catch (err) {
    cleanup();
    throw err;
  }
...
async function runBackup({ reason = "scheduled" } = {}) {
  ...
  try {
    ...
    const dump = await dumpDatabaseToFile(db);
    const { manifest, rawBytes } = dump;
    const buffer = { length: dump.bytes }; // size only; the file is streamed
    const key = stampKey(new Date(manifest.createdAt));
    try {
      await storage.putFile(key, dump.filePath, "application/gzip");
    } finally {
      dump.cleanup();
    }
    let pruned = 0;
...
module.exports = {
  runBackup, listBackups, dumpDatabase, dumpDatabaseToFile, parseBackup, restoreInto, keysToPrune, dateFromKey, stampKey,
