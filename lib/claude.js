...
5. Do not invent new state/federal/ODI items beyond what was provided plus the transfer-pricing addition described above, EXCEPT for the AGM, GRT, and multi-state employee items described in rules 6-8 below, which you must add yourself since they depend on company-specific facts the generic research never had.
6. ANNUAL GENERAL MEETING (AGM): always add a "Mandatory Annual" item named "Annual General Meeting (AGM)", due_date "31 December (Annually)", applicable_to "Corporation (per bylaws)" — or "If required by bylaws" for entity types where an AGM isn't a default statutory requirement (e.g. single-member LLC). Description: the bylaws require the AGM to be held annually by 31 December, and its core agenda item is reviewing the performance targets set for the year against what was actually achieved.
7. GROSS RECEIPTS TAX (GRT): if the profile includes "Average Quarterly Gross Receipts", compare it to $300,000/quarter. If it is at or below $300,000/quarter, add a "Conditional" item named "Gross Receipts Tax (GRT) Filing" with due_date "Exempt — below filing threshold", confidence "medium" (since the exact figure is a state-specific policy, not verified per-state here), and a description stating the company qualifies for the small-business GRT exemption at this revenue level and no GRT filing is currently due, but it should re-check if quarterly receipts rise above $300,000. Regardless of GRT exemption status, ALSO add a separate "Conditional" item named "Local Business License" (if not already present in the generic items) noting that a local city/county business license is required independent of GRT/revenue status.
8. MULTI-STATE EMPLOYEES: if the profile lists employees working from states other than the state of incorporation, add one "Conditional" item PER additional state named "Payroll Withholding & Unemployment Insurance Registration — [State]" (e.g. "— North Dakota"), applicable_to "If employees in this state", description explaining that state income tax withholding registration and State Unemployment Insurance (SUI) registration follow the EMPLOYEE's work state, so the company must register separately in that state even though it is not the state of incorporation — do not merge this into the home-state payroll item. When web search is available to you (it is turned on whenever any such states are present), use it to verify each state's actual withholding/SUI registration agency and any state-specific quirks (e.g. states with no personal income tax) rather than guessing; if search is not available for a given call, mark these items confidence "medium" and note they need verification. If any generic "Sales Tax" conditional item was provided, keep it but rewrite its description to make clear the applicable rate and rules follow the state where nexus/the sale occurs, not a single flat state — sales tax obligations must be evaluated per state of nexus, same as payroll.
${FORMAT_RULES}
...
async function finalizeCompanyCalendar(profile, genericItems, odiGenericItems) {
  ...
  // Also turn on search when the company has employees in states other
  // than its state of incorporation — those states' withholding/SUI
  // registration rules were never covered by the (single-state) generic
  // research, so this is the only point in the pipeline where they can
  // be verified rather than guessed.
  const employeeStates = (profile.employeeStates || []).filter((s) => s && s !== profile.state);
  const needsMultiStatePayroll = employeeStates.length > 0;
  const needsSearch = (needsOdi && !(odiGenericItems && odiGenericItems.length)) || needsMultiStatePayroll;
...
In addition to the above, always apply these company-specific rules:
- ANNUAL GENERAL MEETING (AGM): include a "Mandatory Annual" item named "Annual General Meeting (AGM)", due_date "31 December (Annually)", applicable_to "Corporation (per bylaws)" (or "If required by bylaws" for entity types where an AGM isn't a default statutory requirement). Description: the bylaws require the AGM to be held annually by 31 December, with its core agenda item being a review of the performance targets set for the year against what was actually achieved.
- GROSS RECEIPTS TAX (GRT): if the profile states Average Quarterly Gross Receipts at or below $300,000, include a "Conditional" item "Gross Receipts Tax (GRT) Filing" with due_date "Exempt — below filing threshold" and a description noting the company currently qualifies for the small-business GRT exemption at this revenue level and should re-check if quarterly receipts exceed $300,000; if above $300,000 (or not provided), research and state the actual state GRT filing requirement instead. Regardless of GRT exemption status, separately include a "Conditional" item "Local Business License" noting a local city/county business license is required independent of GRT/revenue status — verify the specific local licensing authority via search where possible.
- MULTI-STATE EMPLOYEES: if the profile lists employees working from states other than the state of incorporation, research and include one "Conditional" item PER additional state, named "Payroll Withholding & Unemployment Insurance Registration — [State]", noting that state income tax withholding registration and State Unemployment Insurance (SUI) registration are governed by the EMPLOYEE's work state, so the company must register separately there even though it is not the state of incorporation. Any Sales Tax conditional item must likewise be described as following the state of nexus/sale, not a single flat state.
${FORMAT_RULES}
