  ...
  "items": [
    {
      ...
      "schedule": "machine-readable form of THIS company's due date, one of: {\"type\":\"annual\",\"month\":7,\"day\":15} | {\"type\":\"multiple\",\"dates\":[{\"month\":4,\"day\":30},{\"month\":7,\"day\":31}]} (several fixed dates a year, e.g. quarterly) | {\"type\":\"monthly\",\"day\":20} | {\"type\":\"anniversary\"} (incorporation anniversary) | {\"type\":\"event\"} (As Triggered). Use the actual statutory due date, never an extension date; day may be \"last\" for the last day of the month.",
      "applicable_to": "short phrase",
...
${JSON.stringify(employeeStateGenericItems, null, 2)}`;
  ...
  const response = await anthropic.messages.create({
    model: MODEL,
    // Was 8000 — too tight for larger profiles (foreign parent + ODI +
    // several employee states can easily produce 25-30+ finalized items),
    // which caused "Model response was truncated (max_tokens)" failures.
    // Matched to fullLiveResearch's cap below, which handles comparably
    // sized output.
    max_tokens: 16000,
...
function buildStaticCompanyItems(profile) {
  ...
  const country = (profile.country || "United States").trim();
  const isUS = ["united states", "usa", "us"].includes(country.toLowerCase());
  ...
  // US-specific — a state-level Gross Receipts Tax concept doesn't exist
  // in most other countries' compliance regimes (Germany's closest
  // equivalent, Gewerbesteuer/trade tax, is already covered as its own
  // proper item in data/presearched-germany.json). Showing this to a
  // non-US company would be actively wrong, not just irrelevant.
...
async function generateCompanyCalendar(profile) {
  ...
  // The US is keyed by (state, entityType) with a separate FEDERAL layer.
  // Every other country's compliance is treated as national rather than
  // state/region-level, so the cache key is the COUNTRY itself (e.g.
  // "GERMANY", matching data/presearched-germany.json) — NOT whatever
  // free-text region the user typed into the state field, which would
  // never reliably match seeded data.
  const state = isUS ? StateCache.normalizeState(profile.state) : StateCache.normalizeState(country);
  ...
  const [stateDoc, federalDoc, odiDoc, employeeStateResult] = await Promise.all([
    ...
    isUS ? StateCache.findOne({ state: "FEDERAL", entityType }) : Promise.resolve(null),
    needsOdi ? OdiCache.findOne({ investorType: profile.odiInvestorType }) : Promise.resolve(null),
    isUS ? resolveEmployeeStateItems(profile) : Promise.resolve({ items: [], anyLive: false }),
  ]);

  const stateFresh = StateCache.isFresh(stateDoc);
  // No separate federal/state split outside the US — don't require a
  // FEDERAL doc that will never exist for a country-keyed entry.
  const federalFresh = isUS ? StateCache.isFresh(federalDoc) : true;
