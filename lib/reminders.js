...
async function runReminderSweep({ now = new Date() } = {}) {
  ...
  console.log(`[reminders] Done: ${calendars.length} calendars, ${clientDigests} client digests, ${teamLines.overdue.length} overdue, ${teamLines.soon.length} due this week, ${teamLines.docs.length} newly stuck on documents, ${paymentReminders} payment reminders.`);
  return { calendars: calendars.length, clientDigests, overdue: teamLines.overdue.length, dueSoon: teamLines.soon.length, docsStuck: teamLines.docs.length, paymentReminders };
}
